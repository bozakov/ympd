...
struct mg_connection {
  ...
  /* Compatibility fields for older Mongoose v5 API used by ympd */
  char *content;                  // Pointer to request/WS payload (NUL-terminated)
  size_t content_len;             // Length of content
  char uri[256];                  // NUL-terminated URI for compatibility
  void *server_param;             // Old server_param field
  void *connection_param;         // Compatibility pointer for per-connection session data
  void *callback_param;           // Compatibility pointer for temporary callback data
  unsigned short local_port;      // Compatibility: local port number
  unsigned short remote_port;     // Compatibility: remote port number
};
...
/* Compatibility layer: provide old Mongoose v5 API symbols expected by ympd.c */
...
/* Digest auth compat (simple stub) */
